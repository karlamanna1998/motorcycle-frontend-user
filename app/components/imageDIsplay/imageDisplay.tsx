...
export default function ImageDisplay({ images, sticky }: { images: string[], sticky: boolean }) {
    ...
    const scrollDown = () => {
        if (imagesListRef.current) {
            imagesListRef.current.scrollTop += 90;
        }
    };
    ...
    return (
        <div className={sticky ? 'image_display_container sticky' : 'image_display_container'}>
            <div className='image_list_wrapper'>
                ...
                <button className='up_btn' onClick={scrollUp}><KeyboardArrowUpIcon /></button>
                <button className='down_btn' onClick={scrollDown}><KeyboardArrowDownIcon /></button>
            </div>
